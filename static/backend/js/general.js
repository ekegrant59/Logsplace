...
function General(){
    ...
    this.General = function() {
        ...
        $(document).on("submit", ".actionForm", function (event) {
            ...
            let redirectUrl = form.data("redirect"); // Get redirect URL (if any)
            ...
            $.post(actionUrl, formData, function (response) {
                setTimeout(() => pageOverlay.hide(), 1000); // Hide overlay after 1s
                
                if (response.status) { 
                    notify(response.message, response.status); // Show toast notification
                    
                    if (response.status === "success" && redirectUrl) {
                        setTimeout(() => window.location.href = redirectUrl, 2000); // Redirect if needed
                    }
                } else {
